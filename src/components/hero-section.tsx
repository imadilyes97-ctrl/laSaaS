...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    if (navEl) {
      tl.fromTo(navEl, { opacity: 0, y: -20 }, { opacity: 1, y: 0, duration: 0.6 })
    }

    // 2. Tag badge
    tl.fromTo('.hero-tag', { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.5 }, '-=0.3')

    // 3. Heading — word-by-word reveal (Awwwards-style)
    ...
    if (headingEl) {
      ...
      words.forEach((word, i) => {
        // Wrap gradient words in gradient spans
      ...
      wordSpans.forEach((word, i) => {
        tl.to(word, {
          ...
          duration: 0.8,
          ease: EASE.out,
        }, `>-${0.7 - i * 0.002}`) // tighter stagger as we go
      })
    }
    ...
    if (subtitleEl) {
      tl.fromTo(subtitleEl, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.6 }, '-=0.1')
    }
  ...
  useRevealStagger('.stat-card', { stagger: 0.12, y: 30 })
  ...
  const showTab = (tab: 'demo' | 'steps') => setActiveTab(tab)
  ...
  return (
    <>
      {/* ═══════ HERO SECTION ═══════ */}
      <section ref={heroRef} className="relative w-full min-h-screen overflow-hidden" style={{ background: '#07050a' }}>
        ...
        <div className="absolute inset-0 pointer-events-none" style={{
          background: `
            radial-gradient(ellipse 80% 60% at 10% 20%, rgba(255, 107, 53, 0.08) 0%, transparent 60%),
            radial-gradient(ellipse 60% 50% at 90% 80%, rgba(124, 58, 237, 0.06) 0%, transparent 60%),
            radial-gradient(ellipse 50% 40% at 50% 50%, rgba(255, 107, 53, 0.03) 0%, transparent 50%)
          `,
        }} />
        ...
        <nav ref={navRef} className="absolute top-0 left-0 right-0 z-30 flex items-center justify-between px-4 sm:px-8 md:px-12 py-5 sm:py-6">
          ...
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#ff6b35] to-[#f72585] flex items-center justify-center shadow-lg shadow-[#ff6b35]/20">
          ...
          {/* Desktop nav */}
          <div className="hidden lg:flex items-center gap-1 bg-[#120f1e]/80 backdrop-blur-lg rounded-full pl-6 pr-1.5 py-1 border border-[rgba(255,107,53,0.1)] shadow-xl shadow-black/20">
            {navLinks.map((link, i) => (
              <a
                key={link.href}
                href={link.href}
                className={`text-sm px-4 py-2 rounded-full transition-all duration-200 ${
                  i === 0
                    ? 'font-medium text-[#ff6b35] bg-[rgba(255,107,53,0.08)]'
                    : 'font-medium text-[#a0a0b8] hover:text-[#fcfcfc] hover:bg-[#1a1630]'
                }`}
            ...
            <button className="ml-2 bg-[#ff6b35] hover:bg-[#e55a2b] text-[#07050a] text-sm font-semibold px-5 py-2.5 rounded-full transition-all duration-200 cursor-pointer shadow-lg shadow-[#ff6b35]/20 hover:shadow-[#ff6b35]/30" style={{ transitionTimingFunction: 'cubic-bezier(0.23, 1, 0.32, 1)' }}>
          ...
          {/* Right actions */}
          <div className="flex items-center gap-4 text-[#a0a0b8]">
            <a
              href={LOGIN_URL}
            ...
            <button
              ...
              className="lg:hidden relative flex items-center justify-center w-10 h-10 rounded-full bg-[#120f1e]/80 backdrop-blur-md border border-[rgba(255,107,53,0.12)] text-[#ff6b35] transition-all duration-300 hover:bg-[#1a1630] cursor-pointer"
        ...
        >
          <div className="absolute inset-0 bg-[#07050a]/80 backdrop-blur-md" />
        </div>
        <div
          className={`lg:hidden fixed top-0 right-0 bottom-0 z-20 w-[85%] max-w-sm bg-[#120f1e]/95 backdrop-blur-2xl shadow-2xl border-l border-[rgba(255,107,53,0.08)] transition-transform duration-500 ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`}
        ...
        >
          <div className="flex flex-col h-full pt-28 px-8 pb-8">
            ...
            >
              <a
                href={LOGIN_URL}
                className="flex items-center gap-2 text-sm font-medium text-[#a0a0b8] hover:text-[#ff6b35] transition-colors"
              >
                <LogIn className="w-4 h-4" />
                Se connecter
              </a>
              <button className="mt-2 w-full bg-[#ff6b35] hover:bg-[#e55a2b] text-[#07050a] text-sm font-semibold px-5 py-3.5 rounded-full transition-all duration-200 cursor-pointer shadow-lg shadow-[#ff6b35]/20">
        ...
        {/* ═══ HERO CONTENT ═══ */}
        <div className="relative z-10 flex flex-col items-center text-center pt-28 sm:pt-32 md:pt-40 px-4 sm:px-6 max-w-5xl mx-auto">
          {/* Tag */}
          <div className="hero-tag inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[rgba(255,107,53,0.1)] border border-[rgba(255,107,53,0.15)] mb-6 sm:mb-8">
            <Sparkles className="w-3.5 h-3.5 text-[#ff6b35]" />
            <span className="text-xs font-medium text-[#ff6b35] tracking-wide">IA Conversationnelle</span>
          </div>

          {/* Heading */}
          <h1
            ref={headingRef}
            className="font-normal leading-[0.92] text-[#fcfcfc] text-[2.5rem] sm:text-[3.5rem] md:text-[5rem] lg:text-[6rem] xl:text-[7rem] max-w-5xl"
            style={{
              fontFamily: "'Instrument Serif', Georgia, serif",
          ...
          {/* Subtitle */}
          <p ref={subtitleRef} className="mt-6 sm:mt-8 text-[#a0a0b8] text-sm sm:text-base md:text-lg leading-relaxed max-w-lg px-2">
          ...
          <div ref={ctaRef} className="flex items-center gap-4 mt-8 sm:mt-10 flex-wrap justify-center">
            <a
              href={LOGIN_URL}
              className="group inline-flex items-center gap-2 bg-[#ff6b35] hover:bg-[#e55a2b] text-[#07050a] text-sm font-semibold px-6 sm:px-8 py-3 sm:py-3.5 rounded-full transition-all duration-200 shadow-xl shadow-[#ff6b35]/20 hover:shadow-[#ff6b35]/30 btn-glow"
            >
            ...
            <button
              ...
              className="group inline-flex items-center gap-2 text-[#fcfcfc] text-sm font-medium border border-[rgba(255,107,53,0.2)] hover:border-[rgba(255,107,53,0.5)] hover:bg-[rgba(255,107,53,0.05)] px-6 sm:px-8 py-3 sm:py-3.5 rounded-full transition-all duration-200 cursor-pointer relative overflow-hidden"
            >
        ...
        <div className="hero-bottom absolute left-0 right-0 bottom-0 z-10 px-4 sm:px-8 md:px-12 pb-6 sm:pb-8">
          <div className="max-w-6xl mx-auto flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div className="max-w-xs">
              <div className="flex items-center gap-2 text-[#a0a0b8] mb-1.5">
                <Sparkles className="w-3.5 h-3.5" />
                <span className="text-xs font-semibold tracking-wider uppercase">FluxEngine</span>
              </div>
              <p className="text-[#6b6b80] text-xs leading-relaxed font-medium">
            ...
            <div className="hidden sm:flex items-center gap-2 text-[#a0a0b8] text-sm">
              ...
              <span className="text-[#6b6b80]">1:35</span>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* ═══════ ADVANTAGES SECTION ═══════ */}
      <section id="avantages" className="relative w-full py-24 sm:py-32 px-4 sm:px-6" style={{ background: '#07050a' }}>
        ...
        <div className="relative max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
            {avantages.map((item, i) => (
              <div
                key={i}
                className="advantage-card group relative overflow-hidden rounded-xl p-6 sm:p-8 transition-all duration-500"
                style={{
      ...
      {/* ═══════ STATS SECTION ═══════ */}
      <section ref={statsRef} className="relative w-full py-16 sm:py-20 px-4 sm:px-6 border-y border-[rgba(255,107,53,0.06)]" style={{ background: '#0c0a14' }}>
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {[
              ...
              <div key={i} className="stat-card text-center p-6 sm:p-8 rounded-xl transition-all duration-300" style={{
                background: 'linear-gradient(180deg, rgba(255, 107, 53, 0.03) 0%, transparent 100%)',
                ...
                >{stat.number}</div>
                <div className="text-xs sm:text-sm text-[#a0a0b8] font-medium">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ═══════ HOW IT WORKS ═══════ */}
      <section id="comment" className="relative w-full py-24 sm:py-32 px-4 sm:px-6" style={{ background: '#0c0a14' }}>
        <div className="absolute inset-0 pointer-events-none" style={{
          background: 'radial-gradient(ellipse 60% 40% at 50% 100%, rgba(124, 58, 237, 0.03) 0%, transparent 60%)',
        }} />
        ...
        <div className="relative max-w-6xl mx-auto">
          <div className="text-center mb-4">
            ...
            <p className="text-[#a0a0b8] text-sm sm:text-base mb-10 max-w-lg mx-auto">
              De la configuration à la première commande, sans complication
            </p>

            {/* Tabs */}
            <div className="inline-flex items-center gap-1 bg-[#120f1e]/80 rounded-full p-1 mb-14 border border-[rgba(255,107,53,0.06)]">
              <button
                className={`px-4 sm:px-5 py-2 text-sm font-medium rounded-full transition-all duration-200 cursor-pointer ${
                  activeTab === 'demo'
                    ? 'text-[#fcfcfc] bg-[rgba(255,107,53,0.1)] shadow-sm'
                    : 'text-[#a0a0b8] hover:text-[#fcfcfc] hover:bg-[#1a1630]'
                }`}
          ...
          {activeTab === 'demo' && (
            <div className="reveal flex flex-col lg:flex-row items-center gap-10 lg:gap-16">
              <div className="lg:w-[45%]">
                ...
                <p className="text-[#a0a0b8] mb-6 text-sm leading-relaxed">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={i} className="flex items-center gap-3">
                      <div className="w-5 h-5 rounded-full bg-[rgba(255,107,53,0.1)] flex items-center justify-center flex-shrink-0">
                        <CheckCircle2 className="w-3 h-3 text-[#ff6b35]" />
                      </div>
                      <span className="text-[#a0a0b8] text-sm">{text}</span>
                    </div>
                  ))}
                </div>
              </div>
